...
describe("OrderSummary", () => {
  ...
  describe("when order is loaded", () => {
    ...
    it("renders order info", () => {
      const { container } = renderWithRouter(() => <OrderSummary useOrderHook={stubUseOrder} />);

      expect(container.innerHTML).toMatch("Product foo");
    });

    it("navigates to main page on button click", () => {
      const { getByText, history } = renderWithRouter(() => <OrderSummary useOrderHook={stubUseOrder} />);
